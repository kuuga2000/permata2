...
<div id="search-results">
	...
	<div id="products-list">
		<?php $ctr = 0; if($product_list) { ?>
			...
			//for diskon.
			if($item->diskonManufaktur!=0 && $item->disc==0){
				$discprice = $item->base_price - ($item->diskonManufaktur * $item->base_price)/100;
			}elseif($item->diskonManufaktur!=0 && $item->disc!=0){
				$discprice = $item->base_price - ($item->disc * $item->base_price)/100;
			}elseif($item->diskonManufaktur==0 && $item->disc!=0){
				$discprice = $item->base_price - ($item->disc * $item->base_price)/100;
			}else{
				$discprice = $item->base_price;
			}
					...
					<div class="product-item <?php echo ($ctr % 5 == 0? "last":"");?>">	<?php 
						...
						<?php 
							...
							<div class="price-original" style="height: 20px;">
								<?php if($item->disc!=0){ 
									echo 'IDR '.$this->currency->idr($item->base_price);
